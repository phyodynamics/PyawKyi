...
export class MicrophoneError extends Error {
  constructor(
    message: string = "Microphone access denied. Please allow microphone permissions.",
  ) {
    super(message);
...
export class AudioProcessingError extends Error {
  constructor(
    message: string = "Failed to process audio. Please try recording again.",
  ) {
    super(message);
...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  NotReadableError:
    "Microphone is busy or unavailable. Please close other apps using the microphone.",
  OverconstrainedError:
    "Microphone settings not supported. Please try a different microphone.",
  SecurityError:
    "Microphone access blocked. Please use HTTPS or allow microphone access.",
...
export function parseGeminiError(statusCode: number, errorBody: string): Error {
  try {
    ...
    // Server returns { error: "message string", status: number }
    // or Gemini returns { error: { message: "...", code: ... } }
    const message =
    ...
    // Use the actual server message instead of generic status-code messages
    return new APIError(
      message,
      statusCode,
